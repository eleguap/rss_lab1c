...
class MinimalPublisher(Node):
    ...
    def timer_callback(self):
        base_to_odom = self.buffer.lookup_transform("odom", "base_link", rclpy.time.Time())

        base_to_odom = self.extract_tf(base_to_odom)
        left_to_base = np.array([[1, 0, 0, 0], [0, 1, 0, 0.05], [0, 0, 1, 0], [0, 0, 0, 1]], dtype=np.float64)
        left_to_odom = self.create_tf("odom", "left_cam", base_to_odom @ left_to_base)

        right_to_left = np.array([[1, 0, 0, 0], [0, 1, 0, -0.10], [0, 0, 1, 0], [0, 0, 0, 1]], dtype=np.float64)
        right_to_left = self.create_tf("left_cam", "right_cam", right_to_left)

        self.broadcaster.sendTransform(left_to_odom)
        self.broadcaster.sendTransform(right_to_left)
        self.get_logger().info(f"Broadcasted: {left_to_odom.child_frame_id}, {right_to_left.child_frame_id}")
